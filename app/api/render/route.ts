...
export async function POST(request: NextRequest): Promise<NextResponse<RenderResponse>> {
  ...
  log.debug('🎨 HTML Render Request Started');
  ...
  try {
    ...
    log.debug('✅ Supabase client initialized for rendering');
    ...
    log.debug('📊 Retrieving template data for:', templateId);
    ...
    if (dbError || !templateData) {
      log.error('❌ Template not found:', dbError);
      return NextResponse.json({
        success: false,
    ...
    for (let i = 0; i < documents.length; i++) {
      ...
      try {
        log.debug(`🎨 Rendering document ${i + 1}/${documents.length}: ${doc.fileName}`);
        ...
        if (instructions && instructions.trim()) {
          log.debug('🧠 Applying AI instructions...');
        ...
        log.debug(`✅ Document ${i + 1} rendered in ${docProcessTime}ms`);
        
      } catch (docError) {
        log.error(`❌ Failed to render document ${i + 1}:`, docError);
    ...
    log.debug('✅ Render operation complete:', {
    ...
    log.error('❌ Render operation failed:', error);
    return NextResponse.json({
      success: false,
